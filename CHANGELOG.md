...
> **Status:** ✅ COMPLETED
> **App Service:** `as-hermesenterprise` (https://as-hermesenterprise.azurewebsites.net)
> **Runtime:** Python 3.12 | gunicorn + uvicorn.workers.UvicornWorker
> **Commit:** `69ba286` → `main`

#### Root Cause
Azure App Service auto-detects `gunicorn application:app` when no startup command is set. The Hermes project has no `application.py` — the entrypoint is `Hermes.Web.backend.main:app` via a custom MetaPathFinder in `Hermes/__init__.py`. Additionally, the previous `startup.sh` script referenced `/home/site/wwwroot/Hermes.Web/` but Oryx extracts the build to `/tmp/<hash>/`, causing all paths in the script to fail.

#### Fixed
- **`.github/workflows/provision-appservice.yml`**
  - `Configure Startup Command` (STEP 7b): Applies `gunicorn --bind=0.0.0.0:8000 --timeout 600 --worker-class uvicorn.workers.UvicornWorker Hermes.Web.backend.main:app` — `--timeout 600` handles cold starts
  - `Configure App Settings` (STEP 7): Now applies to ALL Web Apps (not just new), adds `PYTHONPATH=/home/site/wwwroot:/home/site/wwwroot/Hermes.Web` for belt-and-suspenders import resolution
- **`.github/workflows/deploy.yml`**
  - `startup-command` parameter on `Azure/webapps-deploy@v3` matches provision command
  - Replaced `sleep 30` with readiness polling (30 attempts × 10s = 5 min timeout)
  - Added `Upload Smoke Test Artifact` step (`actions/upload-artifact@v4`, runs `if: always()`)
  - Fixed YAML syntax error (line 174): python3 -c inline code lacked indentation within literal block scalar
  - Fixed OpenAPI contract validation: removed `/openapi.json` from required paths (it is the spec itself, not a user-defined route)
  - Added application identity assertion via OpenAPI `info.title` check (`"Hermes"` in title)
- **Direct Azure fix:** Applied correct startup command directly to `as-hermesenterprise` via Azure CLI

#### Smoke Test — Adversarial E2E (6/6 PASS)

| # | Level | Test | Result |
|---|-------|------|--------|
| 1 | HTTP Status | GET /, /health, /api/version, /api/proyecto, /openapi.json → 200 | ✅ |
| 2 | Adversarial 404 | GET /api/nonexistent-endpoint-xyz → 404 | ✅ |
| 3 | JSON Schema | /health: estado=saludable, aplicacion=Hermes Enterprise Web | ✅ |
| 4 | JSON Schema | /api/version: aplicacion=Hermes Enterprise, commit, version | ✅ |
| 5 | JSON Schema | /api/proyecto: proyecto=Hermes Enterprise, estado=activo | ✅ |
| 6 | OpenAPI | 13 paths, title="Hermes Enterprise - API Publica" | ✅ |

#### Evidence
- **Application Identity:** "Hermes Enterprise - API Publica" (OpenAPI title)
- **Health:** estado=saludable, Python 3.12.13, gunicorn+uvicorn
- **Version:** Hermes Enterprise 2.0.0
- **Direct Azure CLI fix applied:** `az webapp config set --startup-file 'gunicorn --bind=0.0.0.0:8000 --timeout 600 --worker-class uvicorn.workers.UvicornWorker Hermes.Web.backend.main:app'`
- **Artifact:** `rc77c4-smoke-test-<sha>` (auto-generated by pipeline)

#### Key Technical Decisions
1. **`--timeout 600`** in provision command only (not deploy) — handles cold starts where Oryx extracts build to temp directory
2. **`PYTHONPATH`** set to both `/home/site/wwwroot` and `/home/site/wwwroot/Hermes.Web` as fallback for MetaPathFinder
3. **github-sha as default correlationId** — enables tracing from GitHub commit to App Service response
4. **Polling (30×10s)** instead of fixed sleep — adaptive to cold start variability
5. **`actions/upload-artifact@v4`** with `if: always()` — captures evidence even on smoke test failure
