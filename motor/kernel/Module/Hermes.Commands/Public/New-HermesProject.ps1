...
function New-HermesProject {
    ...
    if ($PSCmdlet.ShouldProcess($ProjectPath, "Create Hermes project '$ProjectName'")) {
        Write-Host "[..] Creating project '$ProjectName' at $ProjectPath ..." -ForegroundColor Yellow

        $result = _New-ProjectFromFactory -ProjectPath $ProjectPath -ProjectName $ProjectName `
        ...
        if ($result) {
            ...
            if ($TipoEntorno -eq 'venv') {
                ...
                $venvResult = New-VenvEnvironment -Provider $provider -ProjectPath $ProjectPath -PythonVersion $PythonVersion -ProjectName $ProjectName
                if ($venvResult) {
                    Write-Host "[OK] Virtual environment created at $(Join-Path $ProjectPath '.venv')" -ForegroundColor Green
                }
            }
            ...
            _Create-RequirementsTxt -ProjectPath $ProjectPath
